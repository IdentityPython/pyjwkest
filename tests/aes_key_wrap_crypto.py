...
(For example, these functions are compatible with the openssl functions of the
same name.)
...
def aes_unwrap_key_and_iv(kek, wrapped):
    ...
    R = [""] + [wrapped[i * 8:i * 8 + 8] for i in range(1, n + 1)]
...
def aes_wrap_key(kek, plaintext, iv=0xa6a6a6a6a6a6a6a6):
    n = len(plaintext) / 8
    R = [""] + [plaintext[i * 8:i * 8 + 8] for i in range(0, n)]
    A = iv
